...
Per-category detection rates below are measured against the package's curated unit-test suite (representative attack samples per category). On broader held-out corpora these rates are typically lower — see [tests/adversarial/RESULTS-v4.32.4.md](tests/adversarial/RESULTS-v4.32.4.md) for measured detection on attack corpora and [Known limitations](#what-it-catches-partially-50-80-detection) below.
...
FPR table below measured at v4.19.0, 2026-04-23 (Sanitizer+Encoder pipeline unchanged since). v4.32.0 adversarial corpus: **82.1% recall across 1,182 threat groups / 5,883 payloads** — full breakdown at [tests/adversarial/RESULTS-v4.32.4.md](tests/adversarial/RESULTS-v4.32.4.md). WildChat FPR gate: **494/10,000 = 4.94%** (locked, deterministic regression gate run on every push).
